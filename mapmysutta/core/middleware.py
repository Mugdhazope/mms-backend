...
from mapmysutta.core.device_token import device_id_from_access_token
...
def _device_id_from_authorization(request: HttpRequest) -> str | None:
    ...
    if not token:
        return None
    return device_id_from_access_token(token)
...
class DeviceMiddleware:
    ...
    def __call__(self, request: HttpRequest):
        ...
        bearer_id = _device_id_from_authorization(request)
        device_id = normalize_device_id(
            bearer_id if bearer_id is not None else request.headers.get("X-Device-Id"),
        )
        request.device = None
        if not device_id:
        ...
        if request.device is not None:
            ...
            return JsonResponse(
                {"detail": "Device not found. Identify first."},
                status=403,
            )
        return self.get_response(request)
